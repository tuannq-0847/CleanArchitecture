...
//        }
        if (event.action in arrayOf(MotionEvent.ACTION_DOWN, MotionEvent.ACTION_MOVE)) {
            Log.d("TouchListY", "event y: ${event.y}")
            handleTouchOverScroll(event.rawY)
        } else if (event.action in arrayOf(MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL)) {
            touchList.clear()
//            isNeedSwipeBack = true
        }
        Log.d("TouchList", "onTouch: $touchList")

        if (touchList.size > 1 && isVisibleFirstPosition)
            karlRecycleListener?.onScrollingListener(touchList[1] - touchList[0], event)
        return false
    }
